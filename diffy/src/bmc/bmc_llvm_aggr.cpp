...
bmc_llvm_aggr::
bmc_llvm_aggr( options& o_, std::string suffix, z3::context& z3_, bmc& b_,
...
set_aggr_N(llvm::Function* curr_f) {
  ...
  else tiler_error("DIFF::","Unable to find the program variable N for induction");
}
...
void bmc_llvm_aggr::get_base_spec(std::vector<z3::expr>& post,
  ...
  for(z3::expr arr_var : arr_var_vec) {
    ...
    if(!arr_alloc) tiler_error("DIFF::","Alloca not found for array var");
    ...
    if(base_arr_var) {}
    else tiler_error("DIFF::","Z3 expr not found for alloc");
...
bool bmc_llvm_aggr::verify() {
  if( p->bmc_ds_ptr->spec_vec.empty() ) {
    std::cout << "\n\nNo assertions found!\n\n";
    return true;
  }
  ...
  for(auto it = p->bmc_ds_ptr->spec_vec.begin();
    ...
    if(!check_base_case()) {
      ...
      std::cout << "\n\n" << o.toolNameCaps << "_VERIFICATION_FAILED\n\n";
      return false;
    } else {
      if(unsupported > 0) {
...
bool bmc_llvm_aggr::verify_post(std::vector<z3::expr>& post,
                                std::vector<z3::expr>& body) {
  int iterCount = 0;
  while(iterCount < o.fixpointBound) {
    iterCount++;
    std::vector<z3::expr> hyp;
    std::vector<z3::expr> pre;
    std::vector<z3::expr> base_post;
